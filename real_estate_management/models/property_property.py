...
class Property(models.Model):
    ...
    developer_id = fields.Many2one(
        "res.partner",
        string="Developer",
        help="The developer of the property",
        readonly=False
    )
    ...
    finishing_type = fields.Selection(
        [
            ("ultra_super_lux", "Ultra Super Lux"),],default="ultra_super_lux", string="Finishing Type", help="The finishing type of the property")
    ...
    meter_price = fields.Float(
        ...
        compute="_compute_meter_price",
        store=True,
    )
    floor_number = fields.Char(string="Floor Number", help="The floor number of the property")
    model_number = fields.Char(
        string="Model Number",
        compute='_compute_model_number',
        store=True,
        help="The model number of the property, computed from Floor Number and Name."
    )
    leads_count = fields.Integer(string='Leads Count', compute='_compute_leads_count')

    def action_create_sale_order(self):
        """
        Creates a new sale order for the property.
        """
        self.ensure_one()
        ...
        project_id = self.property_project_id.id if self.property_project_id else False
    ...
    def _compute_leads_count(self):
        """Calculates the number of leads interested in this property."""
        for prop in self:
            # البحث المباشر عن عدد الفرص المرتبطة
            ...
            if leads_data:
                prop.leads_count = leads_data[0]['property_id_count']
            else:
                prop.leads_count = 0

    def action_view_leads(self):
        """
        This function returns an action that displays the leads
        interested in this property.
        """
        self.ensure_one()
        return {
            'name': _('Interested Leads'),
            'type': 'ir.actions.act_window',
            'res_model': 'crm.lead',
    ...
    @api.depends('name', 'floor_number')
    def _compute_model_number(self):
        """
        Computes the model number by concatenating the floor number and the property name.
        """
        for rec in self:
            floor = rec.floor_number or ''
            name = rec.name or ''

            if floor and name:
                rec.model_number = f"{floor}-{name}"
            elif floor:
                rec.model_number = floor
            elif name:
                rec.model_number = name
            else:
                rec.model_number = ''


    @api.depends('unit_price', 'total_sq_feet', 'area_measurement_ids')
    def _compute_meter_price(self):
        """Calculates the price per square meter of the property"""
        for rec in self:
            if rec.total_sq_feet > 0:
                rec.meter_price = rec.unit_price / rec.total_sq_feet
            else:
                rec.meter_price = 0.0
    ...
    def action_property_sale_view(self):
        ...
        return {
            "name": "Property Booking: " + self.code,
            "view_mode": "list,form",
            "res_model": "property.sale",
            "type": "ir.actions.act_window",
